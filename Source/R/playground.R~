...
setwd("~/Work/power/power-failure-prediction-challenge-baseline-master/")
...
# train$縣市行政區 <- paste0(train$CityName, train$TownName, train$VilName)
# train <- left_join(train, pole_type, by = "縣市行政區")

train$key <- paste0(train$CityName, train$TownName, train$VilName) 
train <- left_join(train, pole_type, by="key")

tn_miss <- anti_join(train, pole_type, by="key")

colnames(train)
pole_coles <- c("pole1", "pole2", "pole3", "pole4", "pole5", "pole6", "pole7", "pole8", "pole9", "pole10")
train[,pole_coles][is.na(train[,pole_coles]) == TRUE] <- 0
...
family <- group_by(family, key) %>%
          summarise(household = mean(household_no))
train_new <- left_join(train, family, by="key")
train_new[is.na(train_new$household) == TRUE,]

# TODO: some household is missing
# train_new$household[is.na(train_new$household) == TRUE] <- rep(134958, 183) #高雄市三民區有134958戶
train_new$household[is.na(train_new$household) == TRUE] <- 0
submit_new <- left_join(submit, train_new[, c(3, 14:24)], by = "VilCode")

# 將會用到的颱風資料先選出來
soudelor <- select(train_new, c(1:4, 13:24, 8))
meranti <- select(train_new, c(1:4, 13:24, 12))
